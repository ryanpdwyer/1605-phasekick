...
with h5py.File(file, 'r') as fh:
    for gr_name, priors in grs.items():
        ...
        gr = fh[gr_name]
        out_path = path.join(outdir, gr_name+'-dflive_single.h5')
        data = arrays_to_stan_data(gr['t'].value, gr['mu_df'].value,
                                   gr['sigma_df'].value, gr['N'].value)
        # Use mean of double exponential prior
        priors['mu_tau'] = np.mean(priors['mu_tau'])
        priors['sigma_tau'] = np.mean(priors['mu_tau'])

        pm = p.PhasekickModel('dflive', data, priors=priors)
